...
正式脚本使用 `@grant none`，仅匹配主站与 alt 站。仓库根目录的 `dokidoki.js` 使用固定版本的 jsDelivr 地址；可从 `dokidoki-v0.1.0.0` 标签安装。
...
完整 Monster Lab UI 的真实页面尺寸基线和风险分析见 [`dokidoki UI 重构评估.md`](dokidoki%20UI%20重构评估.md)。0.1.0.0 已完成其中的列表、工具栏、底部操作区和附属面板工作区。
